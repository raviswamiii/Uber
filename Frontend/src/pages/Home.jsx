...
export const Home = () => {
  ...
  try {
    ...
    const response = await axios.get(backendURL + "/rides/getFare", {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden">
      ...
      <div
        className={`absolute bottom-0 w-full bg-white p-2 py-3 transition-all duration-300 ease-in-out overflow-hidden ${
          openVehiclePanel ? "translate-y-0" : "translate-y-full"
        }`}
      >
        <div className="relative flex">
          <h1 className="text-xl font-semibold mb-4">Choose a vehicle</h1>
          <RiArrowDownWideFill
            onClick={() => setOpenVehiclePanel(false)}
            className="absolute right-4 text-2xl text-gray-500"
          />
        </div>
